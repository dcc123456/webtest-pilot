...
| 脚本分享 | 导出 `.wtp.json` 给同事导入，带上关联用例、不带任何密钥；导入只新增不覆盖 |
...
在「脚本」页点某个脚本的 **分享…**，或顶部的 **导出全部**，会生成一个 `.wtp.json` 文件。对方在「脚本」页点 **导入脚本…** 选中文件即可。

**分享包里带什么**：脚本步骤，以及它关联的**测试用例**（步骤描述和预期结果）。带上用例是有意的——只给一串点击步骤，对方不知道这个脚本本来要验证什么。

**绝不带什么**：API Key、密钥的真实值、bridge 令牌、运行历史、定时任务。分享文件很容易被贴进群聊或提交进仓库，所以里面任何敏感内容都等于公开。

密钥以**名字**的形式传递（例如 `LOGIN_PW`），值不会出现在文件里。导入时如果本地缺少某个密钥，插件会直接告诉你缺哪几个，而不是等到运行到一半登录失败才发现。

**导入永远是新增，不会覆盖**。别人机器上的脚本 ID 和你本地的毫无关系，按 ID 合并的话，同事的脚本可能会静默顶掉你自己的。代价是同一个文件导入两次会得到两份——这看得见、可以删；而覆盖造成的数据丢失是不可恢复的。

导入后脚本与用例之间的关联会自动重建，指向新导入的那一份，不会留下指向对方机器的悬空引用。

> 顺带说明和「设置 → 导出全部数据」的区别：那个是**你自己**的完整备份（含设置、定时任务、运行历史），用于换机器或恢复；这里的分享包只装脚本和用例，是给**别人**用的。两者互不通用，喂错了会有明确提示。
...
- [ ] **分享脚本**：点「分享…」下载 `.wtp.json`，确认文件里**没有**密钥真实值、也没有无关用例；换个 Chrome profile 点「导入脚本…」能跑起来
- [ ] **导入不覆盖**：本地先存一个脚本，再导入一个同 ID 的分享包 → 本地脚本仍在，导入的是新的一份
