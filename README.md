...
| 脚本下载 / 导入 | 一键下载 `.json` 到本地（转发/备份用你自己的工具），可再导入；带关联用例、不带任何密钥；导入只新增不覆盖 |
| 失败后分析并续跑 | 脚本失效时智能体接手：说明原因、从断点继续、按用例预期判定；标记为 `recovered` 提醒你修脚本 |
| 本地接口启动测试 | HTTP(REST) + WebSocket + SSE，带 token 鉴权 |
| 用例传输方式 | 「用例编写」自然语言粘贴、Markdown 文件导入、本地接口传入 |
| 对话助手 | 直接分析/操作当前页面，支持技能、操作确认、把操作存成脚本 |
...
2. 点插件图标打开侧边栏，切到「用例编写」。
3. 直接说要验证什么，回车，它会解析成测试用例并可立即运行。如果只是想让 AI 帮你分析或操作页面而不产出用例，用第一个「对话」标签。
...
插件**直接接管你现在这个浏览器**：就在你当前的标签页上操作，不新开窗口。这是它最大的优势——你已登录的会话、Cookie、页面状态全都直接可用，**不需要在测试里先写一遍登录，也不用准备测试账号和数据**。

不用填 URL。用例里不写地址时，插件就用当前标签页；写了地址才会先导航过去。

准确地说，**不写地址时用的一定是「你正在看的那个标签页」**——不会去别的窗口或别的标签页里挑一个。选错页面是破坏性的（往你根本没指的页面里填数据、点按钮），所以这里不做任何猜测：当前页不能用，就直接报错说明，而不是换一个页面接着跑。

模型也会被明确告知「浏览器已经停在目标页面上了，直接 snapshot，不要调 open_url」，避免它自己猜一个地址导航走。
...
> - 或在用例里写明地址（`- url: https://your-site.com/`），插件会自己导航过去。
>
> **另外**：当前页面本身也必须在站点白名单里。不在的话运行会立刻停下，并直接给出可粘贴的那一行（例如 `https://your-site.com/*`），照提示加进设置即可。

### 不只是测试：让它帮你填表、办事

这类「办事」在「**对话**」标签里直接说即可，不需要写「预期结果」：
...
**任何运行都在你现有的浏览器里进行**，包括定时任务和 CI 触发。这是把它做成 Chrome 插件而不是 Playwright 脚本的全部意义：你的登录态、Cookie、本地数据直接沿用，**不需要先写登录步骤，也不用维护测试账号**。
...
只有一种例外：**当前页面确实没法用**（停在新标签页、`chrome://` 页面等），而用例里写了地址——这时会在**你自己的窗口里**开一个**后台标签页**（不抢焦点），跑完自动关掉。

即便这种情况也仍在你的浏览器里，登录态一样有效。插件**不会**关闭你自己的标签页，只关它自己开的那个。

如果当前页不可用、用例又没写地址，插件会直接停下并告诉你打开一个普通网页——而不是随便找个页面动手。
...
侧边栏第一个标签「**对话**」是一个开放式助手，和「用例编写」不同：它不产出测试用例，也没有通过/失败，而是直接对你**正在看的页面**进行观察和操作，做完即止。

可以让它：

- 「分析一下这个页面」「总结一下这篇文章」——读取并总结当前页；
- 「往下滚动，看看还有没有没加载出来的内容」——发现页面没加载完整时自己滚动、点击、再读；
- 「用我的登录信息填写这个表单」——结合技能（见下）自动填写、点击、提交；
- 「点击刷新」「勾选这个选项」「在搜索框输入 xxx」等常见操作。

助手复用了和测试运行同一套页面工具（读取、点击、填写、选择、勾选、按键、滚动、悬停、等待、断言、提取），但**不强制给出结论**：模型说完答案、不再调用工具就结束。

### 操作确认（安全）

助手在你真实、已登录的页面上操作，所以每个对话都可以选择确认模式（输入框上方切换，默认在「设置 → 运行策略」里配）：

- **自动**：模型直接操作，不询问；
- **写操作确认**：只有点击、填写、选择、勾选、按键等**改动页面**的动作会先弹窗让你允许/拒绝，读取和滚动不打扰；
- **全部确认**：每一次工具调用都先问你。

被拒绝的动作会告诉模型「用户拒绝了」，它会改用别的说法或停下，而不是假装执行了。
...
技能是「**操作说明 + 可填表数据**」的组合，在「**设置 → 技能**」里管理：

- **操作说明（instructions）**：告诉模型使用这个技能时该怎么做、按什么顺序；
- **字段（fields）**：为表单里的每个输入给一个匹配标签。值可以直接写，也可以**引用「密钥」里的名称**——后者只传引用，真实值在后台最后一刻替换，不进模型、不进脚本。

在「对话」页顶部选择一个技能后发送，模型就会按其中的说明操作页面，并把字段值（或密钥引用）填进匹配的表单控件。



### 方式一：用「用例编写」标签

在「用例编写」标签里用自然语言描述，插件会解析成用例：
...
定时运行也在你现有的浏览器里进行（沿用登录态），需要新开标签页时用**后台标签页**，不会抢走你正在用的屏幕。
...
点某个脚本的 **下载脚本** 直接得到一个 `.json` 文件；顶部的 **下载全部** 把所有脚本打包成一个文件。不弹预览、不问格式——你要的是文件本身。

拿到文件之后怎么用是你的事：发微信、传飞书、提交进仓库、丢进备份盘都行。插件不做这一步，因为你现有的工具已经做得更好。

反向操作是「脚本」页的 **导入脚本…**，选中文件即可。列表为空时也能导入——那恰恰是最需要它的时候。

**文件里带什么**：脚本步骤，以及它关联的**测试用例**（步骤描述和预期结果）。带上用例是有意的——只给一串点击步骤，看的人不知道这个脚本本来要验证什么。

**绝不带什么**：API Key、密钥的真实值、bridge 令牌、运行历史、定时任务。这种文件很容易被贴进群聊或提交进仓库，所以里面任何敏感内容都等于公开。

密钥以**名字**的形式传递（例如 `LOGIN_PW`），值不会出现在文件里。导入时如果本地缺少某个密钥，插件会直接告诉你缺哪几个，而不是等到运行到一半登录失败才发现。

**导入永远是新增，不会覆盖**。别的机器上的脚本 ID 和你本地的毫无关系，按 ID 合并的话，导入的脚本可能会静默顶掉你自己的。代价是同一个文件导入两次会得到两份——这看得见、可以删；而覆盖造成的数据丢失是不可恢复的。

导入后脚本与用例之间的关联会自动重建，指向新导入的那一份，不会留下指向原机器的悬空引用。

> 和「设置 → 导出全部数据」的区别：那个是**完整备份**（含设置、定时任务、运行历史），用于换机器或恢复；这里只装脚本和用例。两者互不通用，喂错了会有明确提示告诉你该用哪个。
...
脚本是「某一次成功路径的录像」，而页面会变。按钮改了名、表单多了一步，回放就会在中途断掉 —— 这时候把它报成「产品有 bug」是错的。

开启后，回放失败时智能体会接手：

1. **看现在的页面**（snapshot），而不是猜；
2. **说明失败原因**（`diagnose`），这条记录会保存下来，即使续跑最终也失败；
3. **从断掉的那一步继续**跑完剩下的步骤；
4. **逐条断言用例的预期**，再给结论。

### 三条不会让步的规则

**已经执行过的步骤绝不重跑。** 回放可能已经真的下过单、发过邮件。智能体收到的提示里会明确列出「这些已经发生了，不要再做一遍」，并且续跑**不会重新打开起始 URL**（否则登录态和购物车就没了）。这条由测试用变异测试（mutation testing）反向验证过。

**成功与否由用例的预期判定，不由智能体的乐观判定。** 续跑同样要过 `validateVerdict`：没有成功断言的「我觉得通过了」一律判为 `failed`。智能体没有把失败改成成功的权限 —— 如果它认为应用真的坏了，结论仍然是 `failed`。

**定位失效和应用异常，给的提示不一样。**

| 失败类型 | 判断依据 | 给智能体的指引 |
| --- | --- | --- |
| 定位失效（selector） | 元素找不到 | 大概是脚本过期了，去页面上找到那个元素继续；但如果功能真的没了，照实报 failed |
| 应用异常（application） | 元素找到了，动作/断言没成功 | **不要另找一条能过的路** —— 换个按钮点通了就等于把真 bug 藏起来了 |

### 新状态：`recovered`

这类运行既不是 `passed` 也不是 `failed`，而是单独的 **`recovered`（已修复通过）**：**应用是好的，但脚本已经失效。**

单列一个状态是刻意的。如果直接报绿色，就没人知道脚本已经坏了，测试套件会一步一步退化成又慢又贵的智能体运行 —— 每次都要多花一轮模型调用，而且没人察觉。

- 面板里显示为橙色，并给出「该修哪一步、建议怎么改」
- 飞书通知会发（`failure` 策略下也发），但**不会 @ 人**：应用没出问题，不必半夜叫人起来
- **JUnit / CI 退出码默认判为失败**，逼着人去修脚本。确实想先放行的话，用 `--accept-recovered`，或在设置里打开「把 recovered 当作通过」
- 智能体**不会**自动覆写你保存的脚本：它的动作只是对断点的一次修补，不是整条链路的录像，直接存下来会得到一个「从中间开始」的脚本。修复只作为**建议**给人确认

### 默认只对手动运行开启

| 触发方式 | 默认 | 原因 |
| --- | --- | --- |
| 手动 / 对话 | ✅ 开 | 你就在旁边，能立刻读到原因并判断可信度 |
| 定时任务 | ❌ 关 | 半夜没人看。红的被兜成 recovered，脚本可能长期没人修 |
| 本地接口 / CI | ❌ 关 | 流水线只认退出码，需要你先想清楚 recovered 怎么处理 |

纯脚本回放（没有关联用例）不会续跑 —— 没有预期，就没有「跑完」的标准。模型未配置时也不会续跑，而是照实报告原本的失败，不会把测试结果变成配置错误。
...
| 失败续跑默认只对手动运行开启 | 无人值守时把红变绿，等于没人再去修脚本 |
| 续跑不重跑已执行的步骤，也不重新打开起始 URL | 回放可能已经真的下过单；重跑会造成第二次副作用 |
| 续跑的结论同样由 `validateVerdict` 按用例预期判定 | 否则智能体可以靠「我觉得通过了」把真 bug 洗白 |
| 续跑不覆写已保存的脚本 | 会自我改写的脚本可能一路漂移还一路报绿 |
...
- **`failed` 与 `error` 全程分开**。前者是被测应用没达到预期（真发现了问题），后者是工具没能完成（环境/配置问题）。这个区分一直保留到飞书卡片。`cancelled`（人为取消）是第三类，既不是失败也不是错误。
- **运行中的实况归 worker，不归面板**。侧边栏切换 tab 会**卸载**整个组件，所以对话记录一旦放在 React state 里就会连同「切走期间收到的事件」一起消失。智能体的推理和工具调用因此写在 worker 的 `storage.session`（浏览器关闭即清，不像 `local` 会永久堆积），实时事件只是快路径；面板重新挂载时按 `runId + seq` 合并补齐，因此重复合并不会产生重复条目。
- **取消必须传到轮询循环里**。运行的时间几乎全花在 `waitFor` 的轮询上（每步最多 10 秒），只在步骤之间检查信号会让「取消」看起来没反应，所以 `AbortSignal` 挂在 `RunContext` 上随每次 driver 调用下传，等待用的 `sleep` 也可被中断。
...
- [ ] **下载脚本**：点「下载脚本」，浏览器**直接下载**一个 `.json`（不弹预览窗）；确认文件里**没有**密钥真实值、也没有无关用例
- [ ] **下载全部**：顶部按钮下载一个包含全部脚本的文件
- [ ] **导入回来**：换个 Chrome profile 点「导入脚本…」选中刚下载的文件，脚本能正常回放
- [ ] **导入不覆盖**：本地先存一个脚本，再导入一个同 ID 的文件 → 本地脚本仍在，导入的是新的一份
- [ ] **缺密钥提示**：导入一个用到 `secret X` 的脚本但本地没配 → 提示明确列出缺失的密钥名
- [ ] **喂错文件**：把「导出全部数据」的备份喂给「导入脚本…」 → 提示引导去「设置 → 数据导入」，而不是含糊报错
- [ ] **越界拒绝**：让用例访问白名单外的域名 → 运行以拒绝结束，不是静默跳过
- [ ] **密钥不泄漏**：用 `secret X` 登录成功后，检查运行日志和步骤详情里没有明文
- [ ] **截图**：失败步骤自动截图，在运行详情里能看到；缩放（dpr≠1）下裁剪位置正确
- [ ] **接管当前页**：手动运行**不新开任何窗口或标签页**，就在你打开的那个标签页上操作；**免登录**（直接用现有会话）；运行结束后你的标签页**不会被关掉**
- [ ] **定时任务同样接管当前页**：不新开窗口，登录态照样有效
- [ ] **按需截图**：默认只有失败步骤有截图；在用例里明确要求「截图」时，该步骤出现在保存的脚本里，重放时同一位置再次截图
- [ ] **后台标签页兜底**：停在新标签页、且用例写了地址时，会在你自己的窗口里开一个后台标签页（不抢焦点），跑完自动关闭（失败也要关闭）
- [ ] **取消**：运行中点取消，当前步骤结束后停止，状态为 `cancelled`
- [ ] **取消要跟手**：在「等元素出现」这种慢步骤上点取消（默认单步预算 10 秒），应当**1 秒内**就停，而不是等满超时；按钮随即变成「正在取消…」
- [ ] **切 tab 不丢记录**：智能体运行中，从「对话」切到「用例」再切回来 → 之前的推理、工具调用**都还在**，且期间新产生的也补齐了，没有重复
- [ ] **任何 tab 都能停**：运行中切到「脚本」「设置」等页面，顶部标题栏始终有「取消运行」按钮
- [ ] **调用信息足够**：每条工具调用显示 ✓/✗、参数摘要、耗时、第几轮；长时间等模型时有「正在调用模型（第 N/24 轮）」提示，不像卡死
- [ ] **密钥仍不外泄**：用 `secret X` 的步骤，参数摘要里只显示密钥名，看不到明文
- [ ] **定时/bridge 也能取消**：定时任务或通过本地接口启动的运行，同样能被取消（历史上这两条路径注册漏了，取消会报「已结束」）
